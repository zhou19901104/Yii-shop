...
class Nav extends Widget
{
    ...
    public $anchorTemplate = <<<'HTML'
<li class='{class}'>
    <a href="{url}" class="auto">
        <span class="pull-right text-muted">
            <i class="{rightIcon}-o text"></i>
            <i class="{rightIcon} text-active"></i>
        </span>
        <i class="{icon}"></i>
...
HTML;
    ...
    public $dropDownAnchorTemplate = <<<'HTML'
<li class="{class}">
    <a href="{url}" class="auto">
        <i class="{icon}"></i>
        <span>{label}</span>
...
HTML;
    ...
    public function init()
    {
        ...
        if ($this->dropDownCaret === null) {
            $this->dropDownCaret = Html::tag($this->iconTag, '', ['class' => 'i i-circle-sm']);
        }
        Html::addCssClass($this->options, 'nav nav-main');
        $this->options['data-ride'] = "collapse";
    }
    ...
    public function renderItem($item)
    {
        ...
        $rightIcon = isset($item['rightIcon']) ? $item['rightIcon'] : '';
        ...
        return strtr($this->anchorTemplate, [
            '{class}' => $active ? 'active' : '',
            '{url}' => Url::to($url),
            '{label}' => $label,
            '{font}' => $font,
    ...
    {
        return $this->renderDropdownItems($items, $parentItem);
    }
    ...
    protected function renderDropdownItems($items, $parentItem, $html = '') {
        ...
        foreach($items as $item) {
            if (is_array($item) && isset($item['items'])) {
                ...
                $html .= is_string($item) ? $item : strtr($this->dropDownAnchorTemplate, [
                    '{class}' => $this->isItemActive($item) ? 'active' : '',
                    '{url}' => Url::to($item['url']),
                    '{label}' => $item['label'],
